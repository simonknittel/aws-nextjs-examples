...
const Home: NextPage = ({
  ...
  const [users, usersRefreshInProgress, refreshUsers] = useUserGetAll(
    ssrUsers,
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (params.row.archivedDate) {
          return (
            <>
              <RestoreButton params={params} callback={refreshUsers} />
  ...
  const dataGridRowsArchived: GridRowsProp = users.filter(
    (user) => user.archivedDate
  );
    ...
    <>
      ...
      <main>
        ...
        <Box mt={4}>
          ...
          <Accordion
            expanded={showArchivedUsers}
            onChange={() => setShowArchivedUsers(!showArchivedUsers)}
          >
            <AccordionSummary
              aria-controls="archived-users-content"
              id="archived-users-header"
            >
