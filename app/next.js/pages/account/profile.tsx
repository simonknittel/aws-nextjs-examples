...
const Profile: NextPage = ({ me }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const onReset: ReactEventHandler = useCallback(async e => {
    e.preventDefault()
    await doRead()
  }, [ doRead ])

  const onSubmit: ReactEventHandler = useCallback(async e => {
    e.preventDefault()
    await doUpdate()
    await doRead()
  }, [ doRead, doUpdate ])
      ...
      <main>
        ...
        <Box sx={{ mt: 2 }}>
          <form onSubmit={ onSubmit } onReset={ onReset }>
            ...
            <Box sx={{ mt: 2 }}>
              <LoadingButton variant="contained" type="submit" loading={ isLoadingUpdate || isLoadingRead } endIcon={<SendOutlined />}>Submit</LoadingButton>
              <LoadingButton variant="outlined" type="reset" loading={ isLoadingUpdate || isLoadingRead } endIcon={<CancelOutlinedIcon />} sx={{ ml: 1 }}>Reset</LoadingButton>
            </Box>
          </form>
        </Box>
      </main>
